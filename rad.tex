...
\begin{document}
	...
	\begin{abstract}
		Ovaj rad se bavi primenom genetskih algoritama(GA) na problem trgova\v{c}kog putnika(TSP). U prvom delu rada, data je definicija problema trgova\v{c}kog putnika, istorijski pregled problema i algoritama za njegovo resavanje. Tako\dj e su navedeni osnovni pojmovi vezani za genetske algoritme.\\
        Originalni doprinos rada je program koji re\v{s}ava problem trgova\v{c}kog putnika pomocu genetskog algoritma. Algoritam je implementiran u programskom jeziku C++, uz pomo\'{c} biblioteka SDL, OpenGL i GALib. U ovom radu je dat i tehni\v{c}ki opis, na\v{c}in kori\v{s}\'{c}enja programa, kao i pore\dj enje sa rezultatima koji se dobijaju sa drugim algoritmima iz literature.
	\end{abstract}
    ...
    \section{Rezultati}
        ...
        \subsection{Primeri}
        \begin{rem}
           Crvenim linijama je prikazana put generisan algoritmom iz ovog rada, a plavim isprekidanim linijama put generisan Lin-Keringhan heuristikom.
        \end{rem}
        \begin{figure}[H]
        \centering
